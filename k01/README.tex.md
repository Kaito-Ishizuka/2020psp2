...
- File Open error について
  - 実行ファイルが k01 ディレクトリの下の k01.exe だとすると，今，読み込むファイルは，実行ファイルから見て一つ上に登ったディレクトリにある sample というディレクトリの下にある heights_male.csv です．したがって，ファイル名の指定は，
    `../sample/heights_male.csv`
    と指定します．
- printf で出力表示するところが何かおかしい
  - `printf("sample mean:%.21fln", ave);`とありますが，
    - `ln`ではなく，`\n`？　（エルでなく，バックスラッシュ?）
    - `.21f`だと，小数点以下 21 桁で表示することになります．`.2lf`?（イチでなくエル？）
  - `printf("population mean (estimated):%.21f/n", ave);`とありますが，
    - `/n`ではなく`\n`？（スラッシュではなくバックスラッシュ？）
- while 文の中で初回のループでは，n に何が入っていますか？初期化してください．もしゼロが入っていたら，ゼロで割る割り算が出現します．
